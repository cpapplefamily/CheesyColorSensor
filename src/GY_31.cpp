...
GY_31::GY_31(int S2pin, int S3pin, int outpin, int ledpin) {
  S2_Pin  = S2pin;
  S3_Pin  = S3pin;
  OUT_Pin = outpin; 
  LED_Pin = ledpin;
...
int GY_31::getRED() {
   ...
   dat=pulseIn(OUT_Pin,LOW);  //here we wait until "out" go LOW, we start measuring the duration      and stops when "out" is HIGH again
   return dat;
};
